...
def _load_backbone_weights(model, phi):
    """Load an optional backbone checkpoint from a stable project-relative path."""
    weights_dir = Path(os.environ.get(
        "SEGFORMER_RDA_BACKBONE_DIR",
        Path(__file__).resolve().parents[1] / "model_data",
    ))
    weights_path = weights_dir / f"segformer_{phi}_backbone_weights.pth"
    if not weights_path.is_file():
        raise FileNotFoundError(
            f"Backbone weights were not found: {weights_path}. "
            "Download them from the upstream SegFormer release or set "
            "SEGFORMER_RDA_BACKBONE_DIR."
        )
...
class mit_b0(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            ...
            _load_backbone_weights(self, "b0")
...
class mit_b1(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            _load_backbone_weights(self, "b1")
...
class mit_b2(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            _load_backbone_weights(self, "b2")
...
class mit_b3(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            _load_backbone_weights(self, "b3")
...
class mit_b4(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            _load_backbone_weights(self, "b4")
...
class mit_b5(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            _load_backbone_weights(self, "b5")
